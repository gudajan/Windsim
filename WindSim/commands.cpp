...
{
	m_project->removeObject(m_data["name"].toString());
	m_viewer->removeObject3D(m_data["name"].toString());
}

RemoveObjectCmd::RemoveObjectCmd(QString name, Project* project, DX11Widget* viewer, QUndoCommand* parent)
	: QUndoCommand(parent),
	m_data(),
	m_project(project),
	m_viewer(viewer)
{
	m_data = m_project->getObject(name);
